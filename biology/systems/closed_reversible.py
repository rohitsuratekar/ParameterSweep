...
def get_equations(concentrations: list, time: tuple, *args) -> list:
    ...
    p4tase = enzyme_list.get(E_P4TASE)  # type: Enzyme
    p5tase = enzyme_list.get(E_P5TASE)  # type: Enzyme

    pmpi, pi4p, pip2, dag, pmpa, erpa, cdpdag, erpi = concentrations

    d_pmpi = pitp.react_with(erpi) - pi4k.react_with(pmpi) + p4tase.react_with(
        pi4p)
    d_pi4p = pi4k.react_with(pmpi) - pip5k.react_with(
        pi4p) - p4tase.react_with(pi4p) + p5tase.react_with(pip2)
    d_pip2 = pip5k.react_with(pi4p) - plc.react_with(pip2) - p5tase.react_with(
        pip2)
    d_dag = plc.react_with(pip2) - dagk.react_with(dag) + laza.react_with(pmpa)
    d_pmpa = dagk.react_with(dag) - laza.react_with(pmpa) - patp.react_with(
        pmpa)
    d_erpa = patp.react_with(pmpa) - cds.react_with(erpa)
