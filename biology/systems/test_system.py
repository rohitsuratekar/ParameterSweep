...
def get_equations(concentrations: list, time: tuple, *args) -> list:
    ...
    pip5k2 = enzyme_list.get(E_TEST)  # type: Enzyme
    ...
    d_pi4p = pi4k.react_with(pmpi) - pip5k.react_with(
        pi4p) - pip5k2.react_with(pi4p)
    d_pip2 = pip5k.react_with(pi4p) - plc.react_with(pip2) #+ pip5k2.react_with(pi4p)
    d_dag = plc.react_with(pip2) - dagk.react_with(dag)  - sink.react_with(dag) \
            + laza.react_with( pmpa)
