...
def get_equations(concentrations: list, time: tuple, *args) -> list:
    ...
    d_dag = plc.react_with(pip2) - dagk.react_with(dag) + laza.react_with(pmpa) - sink.react_with(dag)
    d_pmpa = dagk.react_with(dag) - laza.react_with(pmpa) - patp.react_with(pmpa)
    d_erpa = patp.react_with(pmpa) - cds.react_with(erpa) + source.react_with(None)  # equal to k value of source
