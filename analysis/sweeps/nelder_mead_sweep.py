...
def sort_best_set(initial_condition, system: str, s1, s2, s3,
                  total_lipid) -> list:
    o1 = get_concentration_profile(system, initial_condition, s1, ode_end_time,
                                   ode_slices)
    o2 = get_concentration_profile(system, initial_condition, s2, ode_end_time,
                                   ode_slices)
    o3 = get_concentration_profile(system, initial_condition, s3, ode_end_time,
                                   ode_slices)
...
def make_initial_set(initial_condition, system: str, kinetics: str,
                     total_lipid) -> list:
...
def mid_point(best, good) -> dict:
    ...
    for key in best_enzyme.keys():
        ...
        mid_enzyme[key] = Enzyme(key, v=vmax, k=km,
                                 kinetics=current_enz1.kinetics)
...
def get_reflection(mid, worst):
    ...
    for key in mid_enzyme.keys():
        ...
        if key == key_rand:
            ...
            vmax = current_enz1.v
            km = current_enz1.k
        reflected_enzyme[key] = Enzyme(key, v=vmax, k=km,
                                       kinetics=current_enz1.kinetics)
